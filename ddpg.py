...
class DDPGAgent:
    def __init__(self, state_dim, action_dim, max_action, actor_lr, critic_lr,
                 buffer_size, batch_size, gamma = 0.99, tau = 0.005):
        
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.main_actor = ActorNetwork(state_dim, action_dim, max_action).to(self.device)
        self.main_critic = CriticNetwork(state_dim, action_dim).to(self.device)
    ...
    def get_action(self, state, exploration_noise=0.1):
        ...
        action = self.main_actor(state_tensor).squeeze(0).cpu().numpy()
    ...
    def learn(self):
        ...
        predicted_actions = self.main_actor(states)
        q_values = self.main_critic(states, predicted_actions)
